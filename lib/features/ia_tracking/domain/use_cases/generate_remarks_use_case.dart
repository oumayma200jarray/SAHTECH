  ...
  List<ClinicalRemark> execute({
    ...
    if (exerciseId.contains('flexion')) {
      if (shoulderImbalance > 15.0) {
        remarks.add(const ClinicalRemark(message: "Attention, ne haussez pas l'épaule. Levez seulement le bras.", severity: RemarkSeverity.warning, metricName: "Épaules"));
      }
      if (trunkLean < -10.0) {
      ...
      if (shoulderImbalance > 15.0) {
        remarks.add(const ClinicalRemark(message: "Attention, gardez l'épaule basse pendant le mouvement.", severity: RemarkSeverity.warning, metricName: "Épaules"));
      }
