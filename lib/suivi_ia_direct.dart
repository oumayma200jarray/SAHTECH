...
import 'package:sahtek/services/pose_detection_service.dart';
import 'package:sahtek/widgets/pose_painter.dart';
import 'package:flutter_tts/flutter_tts.dart';
import 'package:sahtek/controllers/tracking_controller.dart';
import 'package:sahtek/services/claude_ai_service.dart';
import 'package:sahtek/widgets/video_player_widget.dart';
...
class _SuiviIADirectPageState extends State<SuiviIADirectPage>
  ...
  void _checkAndTriggerAI(IATrackingData data) {
    ...
    if ((data.currentValue - _lastAngleForStagnation).abs() < 2.0 && data.currentValue > 10.0) {
      _stagnationCount++;
    } else {
      _stagnationCount = 0;
    }
    ...
    bool cooldownOk = now.difference(_lastAICallTime).inSeconds >= _kAIFeedbackCooldownSec;
    ...
    else if (data.currentValue > data.objective - 30.0 && data.currentValue < data.objective - 10.0) {
      shouldTrigger = true;
      _lastAICallTime = now.add(const Duration(seconds: 10)); // Éviter de répéter trop vite l'encouragement final
    }
